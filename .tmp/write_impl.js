...
var dest = 'C:/Users/ADMIN/OneDrive/Desktop/first_clone_MindFull_ai/backend/src/main/java/com/mindbridge/behavior/feature/window/WindowAggregationServiceImpl.java';

var content = `package com.mindbridge.behavior.feature.window;
...
public class WindowAggregationServiceImpl implements WindowAggregationService {
    ...
    private WindowAggregationResult buildResult(
        ...
        Long checkinCompletedSum7d = sumLong(rows7, UserDailyFeature::getCheckinCompletedCount);
        Long checkinCompletedSum30d = sumLong(rows30, UserDailyFeature::getCheckinCompletedCount);
        ...
        Long riskEventCount7d = sumLong(rows7, UserDailyFeature::getRiskEventCount);
        Long riskEventCount30d = sumLong(rows30, UserDailyFeature::getRiskEventCount);
        long maxRiskDays7 = countDistinctDays(rows7, UserDailyFeature::getMaxRiskLevel);
        long maxRiskDays30 = countDistinctDays(rows30, UserDailyFeature::getMaxRiskLevel);
    ...
    private BigDecimal avgScore(List<UserDailyFeature> rows,
            java.util.function.ToDoubleFunction<UserDailyFeature> getter) {
        java.util.DoubleSummaryStatistics stats = rows.stream()
                .mapToDouble(r -> {
                    BigDecimal val = getter.applyAsDouble(r);
    ...
    private long countDistinctDays(List<UserDailyFeature> rows,
            java.util.function.ToDoubleFunction<UserDailyFeature> getter) {
        return rows.stream()
                .filter(r -> getter.applyAsDouble(r) != null)
    ...
    private Long sumLong(List<UserDailyFeature> rows,
            java.util.function.ToLongFunction<UserDailyFeature> getter) {
        long sum = rows.stream()
                .mapToLong(r -> {
                    Long val = getter.applyAsLong(r);
    ...
    private Integer maxInt(List<UserDailyFeature> rows,
            java.util.function.ToIntFunction<UserDailyFeature> getter) {
        return rows.stream()
                .mapToInt(r -> {
                    Integer val = getter.applyAsInt(r);
